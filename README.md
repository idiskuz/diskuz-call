...
- **Signaling** via MessageBus (channel filtered by recipient). SDP (offer/answer) and ICE candidates are forwarded by the backend; ICE is queued and applied after the remote description.
- Default **STUN** (Google). Optional **TURN** in Admin (JSON) for stricter NATs and corporate/mobile networks.
- **ICE failed:** user message and call end.
...
- `POST /diskuz-call/signal` — send signal (offer, answer, ice_candidate, call_end, call_reject); delivery via MessageBus.
...
- **ICE order:** ICE candidates are queued and applied after the remote description.
- **Autoplay:** The plugin calls `play()` on the remote audio to satisfy browser policies.
- **Microphone:** The user must allow microphone access.
