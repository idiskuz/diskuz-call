...
class DiskuzCallSignalController < ApplicationController
  ...
  def create_incoming_call_notification(callee, caller)
    message_key = "diskuz_call.calling_you"
    title_key = "diskuz_call.incoming_call_title"
    excerpt = I18n.t(message_key, username: caller.username, default: I18n.t("diskuz_call.incoming_call", username: caller.username))
    Notification.create!(
      ...
      data: {
        ...
        "excerpt" => excerpt,
        "description" => excerpt,
        "notification_message" => excerpt,
        "icon" => "phone",
        "data_type" => "incoming_call",
      }.to_json,
    )
