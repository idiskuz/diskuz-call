...
export default apiInitializer("0.8", (api) => {
  ...
  function createHistoryPanel() {
    ...
    historyPanel.innerHTML = `
      <div class="diskuz-history-header">
        ...
        <button id="diskuz-call-history-close" type="button" aria-label="Close" style="border:none;background:none;cursor:pointer;font-size:18px;color:#666;">✕</button>
      </div>
  ...
  function renderHistoryList() {
    ...
    if (!callHistory.length) {
      listEl.innerHTML = `<div class="diskuz-history-empty">No calls yet.</div>`;
      return;
    }
        ...
        return `
          <div class="diskuz-history-item" data-username="${(h.username || "").replace(/"/g, "&quot;")}">
            <div class="diskuz-history-row">
              ...
              <button type="button" class="diskuz-history-username">${h.username}</button>
            </div>
            <div class="diskuz-history-meta">${h.direction} • ${h.result} • ${timeStr}</div>
          </div>
        `;
      })
      .join("");
  ...
  function createFloatingButton() {
    if (!btn) {
      ...
      btn.addEventListener("click", function () {
        if (currentCall.active && currentCall.direction === "incoming" && currentCall.isRinging) {
  ...
  function createWidget() {
    if (!widget) {
      ...
      widget.innerHTML = `
        ...
        <h3 class="diskuz-widget-title" style="display:flex;align-items:center;justify-content:space-between;margin:0 0 10px 0;font-size:18px;font-weight:600;color:#333;">
          Call a friend
          <span class="diskuz-widget-by-inline" style="font-size:11px;color:#888;font-weight:400;">by diskuz.com</span>
        </h3>
  ...
  function showIncomingCallUI(username, avatarTemplate) {
    ...
    const toggleEl = callUI.querySelector(".diskuz-call-toggle-controls");
    const controls = callUI.querySelector(".controls");
    const insertBefore = toggleEl || controls;
    if (callInner) {
      callInner.insertBefore(incomingRow, insertBefore || callInner.firstElementChild);
    }
