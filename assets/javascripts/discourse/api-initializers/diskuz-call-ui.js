...
export default apiInitializer("0.8", (api) => {
  ...
  function messageForSignalReason(reason) {
    const it = document.documentElement.lang === "it";
    switch (reason) {
      case "follow_required":
        return it ? "L'utente deve seguirti per ricevere chiamate." : "User must follow you to receive calls.";
    ...
    if (!widget) {
      ...
      widget.innerHTML = `
        ...
        <label style="display:flex;align-items:center;gap:6px;margin:8px 0;font-size:14px;cursor:pointer;">
          <input type="checkbox" id="diskuz-call-video" style="width:auto;">
      ...
      startBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          const withVideo = !!(widget.querySelector("#diskuz-call-video") && widget.querySelector("#diskuz-call-video").checked);
          log("Starting call to", username, "userId", userId, "video:", withVideo);
          startOutgoingCall(username, userId, data.user.avatar_template, withVideo);
  ...
  function createCallUI() {
    if (!callUI) {
      ...
      callUI.innerHTML = `
        ...
        <div class="call-inner">
          <div class="video-container" style="display:none;">
            <video class="remote-video" autoplay playsinline muted="" style="width:100%;max-height:120px;border-radius:8px;background:#000;"></video>
            <video class="local-preview" autoplay playsinline muted style="position:absolute;bottom:8px;right:8px;width:72px;height:96px;object-fit:cover;border-radius:8px;border:2px solid rgba(255,255,255,0.5);background:#000;"></video>
          </div>
      ...
      const switchCameraBtn = callUI.querySelector(".switch-camera");
      const bothCamerasBtn = callUI.querySelector(".both-cameras");
      ...
      if (switchCameraBtn) {
        switchCameraBtn.addEventListener("click", function () {
          showToast("Use device speaker/earpiece menu for audio. Switch camera: restart call with Video call.");
        });
      }
      if (bothCamerasBtn) {
        bothCamerasBtn.addEventListener("click", function () {
          showToast("Both cameras: coming soon.");
        });
      }
      ...
      if (dragHandle) {
        if (isMobileDevice()) {
          ...
          dragHandle.addEventListener("mousedown", function (e) {
            ...
            dragStartLeft = isNaN(left) ? (window.innerWidth - 300) : left;
  ...
  function updateCallUI(username, avatarTemplate, statusText) {
    ...
    const videoContainer = callUI.querySelector(".video-container");
    const localPreview = callUI.querySelector(".local-preview");
    if (videoContainer && localPreview && rtcLocalStream) {
      const hasVideo = rtcLocalStream.getVideoTracks().length > 0;
      if (hasVideo) {
        localPreview.srcObject = rtcLocalStream;
    ...
    if (!isMobileDevice()) {
      ...
      if (isNaN(left) || isNaN(top)) {
        callUI.style.left = Math.max(0, window.innerWidth - 300) + "px";
  ...
  function closeCallUI() {
    ...
    callUI.classList.remove("open", "diskuz-call-minimized");
    if (proximityOverlay) {
      proximityOverlay.style.display = "none";
    }
    setTimeout(function () {
  ...
  function rtcCreatePeer(targetUserId) {
    ...
    rtcPeer.onconnectionstatechange = () => {
      log("[*] RTCPeerConnection state:", rtcPeer?.connectionState);
    };
  ...
  async function rtcMakeOffer(targetUserId) {
    ...
    iceCandidateQueue = [];
    const withVideo = !!currentCall.withVideo;
    const stream = await rtcStartLocalMedia(withVideo);
    if (!stream) {
      ...
      if (sendPromise && typeof sendPromise.then === "function") {
        sendPromise.then(
          ...
          (err) => {
            ...
            showToast(messageForSignalReason(reason));
            endCurrentCall("rejected");
          }
        );
      }
  ...
  async function rtcSendAnswer() {
    ...
    const sdpStr = (currentCall.offerSdp && currentCall.offerSdp.sdp) ? currentCall.offerSdp.sdp : "";
    const offerHasVideo = /m=video/.test(sdpStr);
    log("[CALLEE] rtcSendAnswer: start, userId=", currentCall.userId, "offerHasVideo:", offerHasVideo);
    const stream = await rtcStartLocalMedia(offerHasVideo);
    if (!stream) {
  ...
  function rtcHandleIce(data) {
    ...
    try {
      ...
      log("[*] rtcHandleIce: added ICE candidate");
    } catch (e) {
  ...
  function startOutgoingCall(username, userId, avatarTemplate, withVideo) {
    log("startOutgoingCall", username, userId, "video:", !!withVideo);
    if (userId != null && userId === currentUserId) {
    ...
    currentCall.withVideo = !!withVideo;
