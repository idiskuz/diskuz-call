...
/**
 * Inizializzazione completa UI diskuz-call. Chiamata dal modulo lazy diskuz-call-full.
 * Se il floating button esiste già (bootstrap), registra solo DiskuzCallToggleWidget.
 */
export function initUI(api) {
  log("diskuz-call-ui initUI running (lazy)");
  ...
  function createFloatingButton() {
    const existingBtn = document.getElementById("diskuz-call-btn");
    if (existingBtn) {
      btn = existingBtn;
      window.DiskuzCallToggleWidget = toggleWidget;
      return;
    }
    if (!btn) {
    ...
    ajax("/diskuz-call/status")
      .then((data) => {
        if (data.enabled !== true) {
          ...
          const bootstrapBtn = document.getElementById("diskuz-call-btn");
        ...
        window.__DiskuzCallFullLoaded = true;
        toggleWidget();
      })
      .catch(() => {
